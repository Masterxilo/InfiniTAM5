...
static struct func {
	...
	const char  *f_name;
		...
		{ 1, 0, _tr0, "createScene" },
		{ 1, 0, _tr1, "getSceneVoxelSize" },
		{ 2, 0, _tr2, "serializeScene" },
		{ 2, 0, _tr3, "deserializeScene" },
		{ 1, 0, _tr4, "countVoxelBlocks" },
		{ 2, 0, _tr5, "getVoxelBlock" },
		{ 1, 2, _tr6, "putVoxelBlock" },
		{ 2, 0, _tr7, "meshScene" },
		{ 2, 0, _tr8, "initFineFromCoarse" },
		{ 2, 0, _tr9, "computeArtificialLighting" },
		{ 1, 0, _tr10, "estimateLighting" },
		{ 2, 0, _tr11, "buildSphereScene" },
		{ 2, 2, _tr12, "renderScene" },
		{ 2, 2, _tr13, "processFrame" },
		{ 0, 0, _tr14, "assertFalse" },
		{ 0, 0, _tr15, "assertGPUFalse" }
		};
...
#endif
{
	...
	if (_res) _res = _definepattern(mlp, (char *)"createScene[voxelSize_Real]", (char *)"{ voxelSize }", 0);
	if (_res) _res = _definepattern(mlp, (char *)"getSceneVoxelSize[id_Integer?NonNegative]", (char *)"{ id }", 1);
	if (_res) _res = _definepattern(mlp, (char *)"serializeScene[id_Integer?NonNegative, fn_String]", (char *)"{ id, fn }", 2);
	if (_res) _res = _definepattern(mlp, (char *)"deserializeScene[id_Integer?NonNegative, fn_String]", (char *)"{ id, fn }", 3);
	if (_res) _res = _definepattern(mlp, (char *)"countVoxelBlocks[id_Integer?NonNegative]", (char *)"{ id }", 4);
	if (_res) _res = _definepattern(mlp, (char *)"getVoxelBlock[id_Integer?NonNegative, i_Integer?Positive]", (char *)"{ id, i }", 5);
	if (_res) _res = _definepattern(mlp, (char *)"putVoxelBlock[id_Integer?NonNegative, voxelBlockData : { {_,_,_} (*pos*), {__List} (*8^3 voxels' data*) }]", (char *)"{ id, voxelBlockData }", 6);
	if (_res) _res = _definepattern(mlp, (char *)"meshScene[id_Integer?NonNegative, fn_String]", (char *)"{ id, fn }", 7);
	if (_res) _res = _definepattern(mlp, (char *)"initFineFromCoarse[idFine_Integer, idCoarse_Integer] /; (idFine != idCoarse)", (char *)"{ idFine, idCoarse }", 8);
	if (_res) _res = _definepattern(mlp, (char *)"computeArtificialLighting[id_Integer?NonNegative, dir : {_,_,_}?numericVectorQ]", (char *)"{ id, dir}", 9);
	if (_res) _res = _definepattern(mlp, (char *)"estimateLighting[id_Integer?NonNegative]", (char *)"{ id }", 10);
	if (_res) _res = _definepattern(mlp, (char *)"buildSphereScene[id_Integer?NonNegative, rad_Real]", (char *)"{ id, rad }", 11);
	if (_res) _res = _definepattern(mlp, (char *)"renderScene[         id_Integer?NonNegative,          shader_String,         (* Manual *)         poseWorldToView_?PoseMatrixQ,         rgbIntrinsics : NamelessIntrinsicsPattern[]     ]", (char *)"{ id, shader, poseWorldToView, rgbIntrinsics }", 12);
	if (_res) _res = _definepattern(mlp, (char *)"processFrame[doTracking : 0|1     , id_Integer?NonNegative     (* Manual *)     , rgbaByteImage_ /;TensorQ[rgbaByteImage, IntegerQ] && Last@Dimensions@rgbaByteImage == 4     , depthData_?NumericMatrixQ     , poseWorldToView_?PoseMatrixQ     , intrinsicsRgb : NamelessIntrinsicsPattern[]     , intrinsicsD : NamelessIntrinsicsPattern[]     , rgbToDepth_?PoseMatrixQ     ]", (char *)"{ doTracking, id, rgbaByteImage, depthData, poseWorldToView, intrinsicsRgb, intrinsicsD, rgbToDepth }", 13);
	if (_res) _res = _definepattern(mlp, (char *)"assertFalse[]", (char *)"{ }", 14);
	if (_res) _res = _definepattern(mlp, (char *)"assertGPUFalse[]", (char *)"{ }", 15);
...
#endif
{
	return _WSDoCallPacket( mlp, _tramps, 16);
} /* WSDoCallPacket */
