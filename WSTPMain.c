...
static struct func {
	...
	const char  *f_name;
		...
		{ 2, 0, _tr4, "meshScene" },
		{ 2, 0, _tr5, "initFineFromCoarse" },
		{ 2, 0, _tr6, "computeArtificialLighting" },
		{ 1, 0, _tr7, "estimateLighting" },
		{ 2, 0, _tr8, "buildSphereScene" },
		{ 2, 2, _tr9, "renderScene" },
		{ 2, 2, _tr10, "processFrame" },
		{ 0, 0, _tr11, "assertFalse" },
		{ 0, 0, _tr12, "assertGPUFalse" }
		};

static const char* evalstrs[] = {
	"BeginPackage[\"InfiniTAM`\", {\"numerics`\"}] (* need to state every",
	"thing but System` that I need *)",
	(const char*)0,
	"Begin@\"`Private`\" (* create everythin in InfiniTAM`Private`* *)",
	(const char*)0,
	"End[]",
	(const char*)0,
	"EndPackage[]",
...
#endif
{
	...
	if (_res) _res = _definepattern(mlp, (char *)"getSceneVoxelSize[id_Integer]", (char *)"{ id }", 1);
	if (_res) _res = _definepattern(mlp, (char *)"serializeScene[id_Integer, fn_String]", (char *)"{ id, fn }", 2);
	if (_res) _res = _definepattern(mlp, (char *)"deserializeScene[id_Integer, fn_String]", (char *)"{ id, fn }", 3);
	if (_res) _res = _definepattern(mlp, (char *)"meshScene[id_Integer, fn_String]", (char *)"{ id, fn }", 4);
	if (_res) _res = _definepattern(mlp, (char *)"initFineFromCoarse[idFine_Integer, idCoarse_Integer] /; (idFine != idCoarse)", (char *)"{ idFine, idCoarse }", 5);
	if (_res) _res = _definepattern(mlp, (char *)"computeArtificialLighting[id_Integer, dir : {_,_,_}?numericVectorQ]", (char *)"{ id, dir}", 6);
	if (_res) _res = _definepattern(mlp, (char *)"estimateLighting[id_Integer]", (char *)"{ id }", 7);
	if (_res) _res = _definepattern(mlp, (char *)"buildSphereScene[id_Integer, rad_Real]", (char *)"{ id, rad }", 8);
	if (_res) _res = _definepattern(mlp, (char *)"renderScene[         sceneId_Integer,          shader_String,         (* Manual *)         poseWorldToView_?poseMatrixQ,         rgbIntrinsics : NamelessIntrinsicsPattern[]     ]", (char *)"{ sceneId, shader, poseWorldToView, rgbIntrinsics }", 9);
	if (_res) _res = _definepattern(mlp, (char *)"processFrame[doTracking : 0|1     , sceneId_Integer     (* Manual *)     , rgbaByteImage_ /;TensorQ[rgbaByteImage, IntegerQ] && Last@Dimensions@rgbaByteImage == 4     , depthData_?NumericMatrixQ     , poseWorldToView_?poseMatrixQ     , intrinsicsRgb : NamelessIntrinsicsPattern[]     , intrinsicsD : NamelessIntrinsicsPattern[]     , rgbToDepth_?poseMatrixQ     ]", (char *)"{ doTracking, sceneId, rgbaByteImage, depthData, poseWorldToView, intrinsicsRgb, intrinsicsD, rgbToDepth }", 10);
	if (_res) _res = _definepattern(mlp, (char *)"assertFalse[]", (char *)"{ }", 11);
	if (_res) _res = _definepattern(mlp, (char *)"assertGPUFalse[]", (char *)"{ }", 12);
